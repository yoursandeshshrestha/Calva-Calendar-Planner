...
function mapGoogleEvent(account: ConnectedAccount, event: GoogleCalendarEvent) {
  const meetLink =
    event.hangoutLink ??
...
async function fetchEventsForAccount(
  ...
  return events.map((event) => mapGoogleEvent(account, event))
}
  ...
  try {
    ...
    for (const account of accounts as ConnectedAccount[]) {
      try {
        ...
        if (accountId) throw accountError
      }
    }
